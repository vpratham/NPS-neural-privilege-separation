...
CONFIG = {
    ...
    # 1) NFW_ROOT environment variable
    # 2) Google Drive mount when running interactively in Colab
    # 3) current working directory / NFW-001 for Colab CLI or local runs
    "project_root": None,
    "project_root_rel": "NFW-001",
    "split_file_rel": "splits/necent_nfw_splits_v2.json",
    "probe_artifact_dir_rel": "artifacts/exp017",
    "runs_dir_rel": "runs",
    "dataset_cache_rel": "cache/necent_train.parquet",
...
# Stage 2a: resolve project storage for Colab notebook, Colab CLI, and local runs.
def resolve_project_root(config):
    env_root = os.environ.get("NFW_ROOT")
    if env_root:
        root = Path(env_root).expanduser().resolve()
        print(f"Using NFW_ROOT: {root}")
        return root

    # Interactive Colab: use Drive if already mounted or mount it when possible.
    if os.environ.get("COLAB_RELEASE_TAG"):
        drive_root = Path("/content/drive")
        try:
            if not drive_root.exists() or not any(drive_root.iterdir()):
                from google.colab import drive
                print("Interactive Colab detected; mounting Google Drive...")
                drive.mount(str(drive_root), force_remount=False)
            candidate = drive_root / "MyDrive" / "NFW-001"
            if candidate.exists():
                print(f"Using Google Drive project root: {candidate}")
                return candidate
        except Exception as exc:
            print(f"Google Drive unavailable ({exc}); falling back to local storage.")

    # Colab CLI and local execution: use a persistent working directory.
    root = (Path.cwd() / config["project_root_rel"]).resolve()
    print(f"Using local project root: {root}")
    return root

PROJECT_ROOT = resolve_project_root(CONFIG)
CONFIG["project_root"] = str(PROJECT_ROOT)
SPLIT_FILE = PROJECT_ROOT / CONFIG["split_file_rel"]
PROBE_DIR = PROJECT_ROOT / CONFIG["probe_artifact_dir_rel"]
RUNS_DIR = PROJECT_ROOT / CONFIG["runs_dir_rel"]
RUN_DIR = RUNS_DIR / CONFIG["run_id"]
DATASET_CACHE = PROJECT_ROOT / CONFIG["dataset_cache_rel"]
...
print("\nSearching project root for Exp017 files...")
